...
from mg24_proto import DEcwHisPErMsG_pb2 , MajoRLoGinrEs_pb2 , PorTs_pb2 , MajoRLoGinrEq_pb2 , sQ_pb2 , Team_msg_pb2, RemoveFriend_Req_pb2, GetFriend_Res_pb2, spam_request_pb2, devxt_count_pb2, dev_generator_pb2, kyro_title_pb2, room_join_pb2
from protobuf_decoder.protobuf_decoder import Parser
from MG24xC4 import * ; from xHeaders import * ; from xMG24 import *
...
ADMIN_UID = "3112644964"
server2 = "BD"
key2 = "mg24"
BYPASS_TOKEN = "your_bypass_token_here"
WHITELIST_ONLY = False
bot_enabled = True
BOT_OWNER_UID = 3112644964  
PLAYER_NAME_CACHE = {}
...
async def xghost_pakcet(player_id , secret_code ,K , V):
    fields = {
        1: 61,
        2: {
            1: int(player_id),  
...
async def ghost_join_packet(team_code, player_name, key, iv):
    """Create ghost join packet directly without external API"""
    try:
        fields = {
            1: 61,  # Ghost join packet type
            2: {
                1: 13777734363,  # Bot UID
                2: {
                    1: 13777734363,  # Bot UID again
                    2: int(time.time()),
                    3: f"[b][c][FF0000]{player_name}",
                    5: 12,
                    6: 9999999,
                    7: 1,
                    8: {
                ...
                3: str(team_code),  # Team code
            }
        }
        
        # Encrypt and create final packet
        packet = await CrEaTe_ProTo(fields)
        packet_hex = packet.hex()
...
def start_rmspam(uid):
    """Call localhost API to start RM spam"""
    url = f"http://localhost:5008/start_spam?uid={uid}&duration=1"
    try:
        res = requests.get(url, timeout=10)
        if res.status_code == 200:
            data = res.json()
            message = data.get("message", "Spam started successfully!")
            return f"[B][C][00FF00]✅ {message}"
        else:
            return f"[B][C][FF0000]❌ API Error (Status: {res.status_code})"
    except requests.exceptions.RequestException:
        return "[B][C][FF0000]❌ Could not connect to RM Spam API (port 5008). Is it running?"
    except Exception as e:
...
def get_level_info(player_id):
    url = f"http://localhost:5006/level?uid={player_id}"
...
"""
        ...
        # Use local API
        url = f"http://localhost:5005/remove_friend?uid={uid}&password={password}&friend_uid={target_uid}"
...
def check_ban(uid):
    try:
        url = f"http://localhost:5004/ban?uid={uid}"
        res = requests.get(url, timeout=10)
...
def talk_with_ai(question):
    url = f"http://localhost:5001/ask?key=MG24_GAMER&question={question}"
    res = requests.get(url)
    if res.status_code == 200:
        data = res.json()
...
async def EncRypTMajoRLoGin(open_id, access_token, region="IND", lang_code="en"):
    # মিনিমাম ডিভাইস ইনফো - শুধু মোবাইলের মতো দেখাবে
    ...
    packet = b''
    packet += _encode_length_delimited(3, now_str)           # event_time
    packet += _encode_length_delimited(4, "free fire")      # game_name
    packet += _encode_length_delimited(7, "1.123.1")          # client_version
    packet += _encode_length_delimited(20, ip)              # client_ip
    packet += _encode_length_delimited(21, lang_code)       # language
    packet += _encode_length_delimited(22, open_id)         # open_id
    packet += _encode_length_delimited(26, region.upper())  # lock_region
    packet += _encode_length_delimited(29, access_token)    # access_token
    
    # ✅ মোবাইলের মতো দেখাতে → 2 (Android)
    packet += _encode_varint_field(38, 2)                   # platform_id (2 = Android)
    
    # ✅ মোবাইলের মতো দেখাতে → 1 (Handheld)
    packet += _encode_varint_field(49, 1)                   # device_type (1 = Handheld)
...
async def send_all_titles_sequentiallly(uid, chat_id, key, iv, region, chat_type):
    ...
    # Send initial message
    start_msg = f"""[B][C][00FF00] Noobde MG24 GAMER ya meku agar tu noob bolra toh tu g a y hai
...
📝 Examples:
/bio Hello World!
/bio 🤖 Bot by MG24 GAMER
/bio Level 70 | Pro Player
/bio Add me: MG24 GAMER
...
# ==================== /room COMMAND (API BASED) ====================
                        if inPuTMsG.strip().startswith('/room'):
                            print('Processing /room command')
                            
                            parts = inPuTMsG.strip().split()
                            
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ Usage: /room (room_id) [password]\nExample: /room 123456 0000\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                room_id = parts[1]
                                ...
                                initial_msg = f"[B][C][00FF00]🚀 Joining room {room_id}...\n"
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                
                                try:
                                    # API Call (packet এর বদলে)
                                    url = f"http://localhost:5011/room?id={room_id}&password={password}"
                                    res = requests.get(url, timeout=10)
                                    
                                    if res.status_code == 200:
                                        data = res.json()
                                        success_msg = f"[B][C][00FF00]✅ Room Joined!\n🏠 Room: {room_id}\n"
                                        await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                    else:
                                        error_msg = f"[B][C][FF0000]❌ API Error (Status: {res.status_code})\n"
                                        await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                        
                                except Exception as e:
                                    error_msg = f"[B][C][FF0000]❌ Failed: {str(e)[:50]}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/rmspam'):
                            print('Processing rmspam command')
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ Usage: /rmspam <uid>\nExample: /rmspam 88😞04😞135😞237"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                initial_msg = f"[B][C][00FFFF]🚀 Starting Room Spam for {xMsGFixinG(target_uid)}..."
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                                result = start_rmspam(target_uid)
                                await safe_send_message(response.Data.chat_type, result, uid, chat_id, key, iv)

#CREATE ROOM COMMAND 
                        if inPuTMsG.strip().startswith('/createroom'):    
                            print('Processing leave room command')    
    
                            initial_msg = f"[B][C][FFFF00]🚪 Creating room...\n"    
                            await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)    
    
                            try:    
                                # Leave the current room    
                                leave_packet = await Mg24CreateRoom("[B][FF0000]MG24", key, iv)    
                                await SEndPacKeT(whisper_writer, online_writer, 'OnLine', leave_packet)    
                
                                success_msg = f"[B][C][00FF00]✅ Successfully created the room!\n"    
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)    
                
                            except Exception as e:    
                                error_msg = f"[B][C][FF0000]❌ Failed to leave room: {str(e)}\n"    
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)

# ==================== /joinroom COMMAND (Password Optional) ====================
                        if inPuTMsG.strip().startswith('/joinroom'):
                            print('Processing joinroom command (password optional)')

                            parts = inPuTMsG.strip().split()

                            # At least room_id needed
                            if len(parts) < 2:
                                error_msg = (
                                    "[B][C][FF0000]Wrong Format Usage:\n"
                                    "/joinroom <room_id> [password]\n"
                                    "Example: /joinroom 123😞456\n"
                                    "Example: /joinroom 123😞456 00😞00\n"
                                    "(If no password given, bot joins with empty password – no password room)"
                                )
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                continue

                            room_id = parts[1]
                            password = parts[2] if len(parts) >= 3 else ""   # empty string = no password

                            # Validate room_id
                            if not room_id.isdigit():
                                error_msg = "[B][C][FF0000]❌ Room ID must be a number!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                continue

                            # Sending status
                            initial_msg = f"[B][C][00FF00]🚀 Joining room {xMsGFixinG(room_id)}..."
                            if password:
                                initial_msg += f"\n🔑 Password: {xMsGFixinG(password)}"
                            else:
                                initial_msg += "\n🔓 No password (open room)"
                            await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)

                            try:
                                # Use the internal Mg24JoinRomm function (imported from xMG24)
                                join_packet = await Mg24JoinRomm(room_id, password, key, iv)

                                if join_packet and online_writer:
                                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', join_packet)

                                    success_msg = f"[B][C][00FF00]✅ Joined room {xMsGFixinG(room_id)} successfully!"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                else:
                                    await safe_send_message(
                                        response.Data.chat_type,
                                        "[B][C][FF0000]❌ Failed to create join packet!",
                                        uid, chat_id, key, iv
                                    )
                            except Exception as e:
                                error_msg = f"[B][C][FF0000]❌ Room join error: {str(e)[:50]}"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)

# ==================== /rminv COMMAND (Room Invite using xMG24's Mg24RoomInv) ====================
                        if inPuTMsG.strip().startswith('/rminv'):
                            print('Processing /rminv (room invite) command')
                            ...
                            if len(parts) < 2:
                                error_msg = (
                                    "[B][C][FF0000]❌ Usage:\n"
                                    "/rminv <uid>\n"
                                    "Example: /rminv 1444😞44😞440😞04\n\n"
                                    "[FFFFFF]ℹ️ The bot must already be inside a custom room to send a room invite."
                                )
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                continue
                            ...
                            if not target_uid.isdigit():
                                error_msg = "[B][C][FF0000]❌ UID must be a number!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                continue

                            initial_msg = f"[B][C][00FF00]📨 Sending room invite to {xMsGFixinG(target_uid)}...\n"
                            await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)

                            try:
                                # ✅ FIX: Add await to Mg24RoomInv (it returns a coroutine)
                                invite_packet = await Mg24RoomInv(key, iv, target_uid)

                                if invite_packet and online_writer:
                                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', invite_packet)
                                    success_msg = f"[B][C][00FF00]✅ Room invite sent to {xMsGFixinG(target_uid)}!\n"
                                    await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                else:
                                    await safe_send_message(
                                        response.Data.chat_type,
                                        "[B][C][FF0000]❌ Failed to create room invite packet!",
                                        uid, chat_id, key, iv
                                    )
                            except Exception as e:
                                error_msg = f"[B][C][FF0000]❌ Error: {str(e)[:50]}\n"
...
# ==================== /dhadha COMMAND ====================
                        ...
                        if inPuTMsG.strip().startswith("/worldjoin"):
                            print(f"Processing worldjoin command: {response.Data.chat_type}")

                            parts = inPuTMsG.strip().split()

                            try:
                                # HELP / LIST
                                if len(parts) == 2 and parts[1].lower() in ("help", "list"):
                                    help_msg = f"[B][C][00FF00]🌍 WORLD JOIN SYSTEM\n"
                                    help_msg += f"[FFFFFF]────────────────\n"
                                    help_msg += f"[00FF00]📌 USAGE:\n"
                                    help_msg += f"[FFFFFF]/worldjoin CODE\n"
                                    help_msg += f"[FFFFFF]────────────────\n"
                                    help_msg += f"[00FF00]📖 EXAMPLE:\n"
                                    help_msg += f"[FFFFFF]/worldjoin ABC123\n"
                                ...
                                if len(parts) < 2:
                                    ...
                                    error_msg += f"[FFFFFF]Usage: /worldjoin CODE\n"

                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                    continue

                                WORLD_OWNER = 14444523057
                                code = parts[1]

                                # LOADING
                                loading_msg = f"[B][C][00FF00]🌍 Joining World...\n"
                                await safe_send_message(response.Data.chat_type, loading_msg, uid, chat_id, key, iv)

                                # PACKET BUILD + SEND
                                H = await GenJoinGlobaL(WORLD_OWNER, code, K, V)
                                await SEndPacKeT(whisper_writer, online_writer, 'OnLine', H)

                                # SUCCESS
                                success_msg = f"[B][C][00FF00]✅ WORLD JOIN SENT!\n"
                                success_msg += f"[00FF00]👤 Owner: {WORLD_OWNER}\n"
                                success_msg += f"[00FF00]🔑 Code: {code}\n"
                                success_msg += f"[00FF00]🚀 Status: Success\n"

                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)

                            except Exception as e:
                                error_msg = f"[B][C][FF0000]❌ WORLD JOIN FAILED!\n"
                                error_msg += f"[FFFFFF]Error: {str(e)}\n"
                        ...
                        # ==================== GHOST COMMAND - FIXED VERSION ====================
                        if inPuTMsG.strip().startswith('/ghost'):
                            print('👻 Ghost command triggered')
...
[FF69B4]═══════════════════════
[00FF00]📌 Usage:
[FFFFFF]  /ghost [team_code] [name]

[00FF00]📝 Examples:
[FFFFFF]  /ghost 5190762
[FFFFFF]  /ghost 5190762 "MG24"
...
[FF69B4]═══════════════════════"""
                            ...
                            ghost_name = "MG24 GAMER"
                            if len(parts) >= 3:
                                ghost_name = ' '.join(parts[2:])
                            
                            # Bot's UID
                            bot_uid = 13777734363
                            
                            await safe_send_message(response.Data.chat_type, 
                                f"[B][C][00FF00]👻 Ghost joining [FFFFFF]{team_code}[00FF00]...", 
                                uid, chat_id, key, iv)
                            
                            try:
                                # DIRECT GHOST PACKET CREATION (without calling external function)
                                fields = {
                                    1: 61,
                                    2: {
                                        1: int(bot_uid),  
                                ...
                                # Generate final encrypted packet
                                ghost_packet = await GeneRaTePk(packet_hex, '0515', key, iv)
                                
                                if ghost_packet:
                                    # Send ghost packet
                                    await SEndPacKeT(whisper_writer, online_writer, 'OnLine', ghost_packet)
                                    print(f"✅ Ghost packet sent for team: {team_code}")
                                    await asyncio.sleep(0.5)
                                    
                                    # Send ghost chat message (optional)
                                    try:
                                        chat_msg = f"[B][C][FF00FF]👻 {ghost_name} joined as ghost!"
                                        chat_packet = await xSEndMsgsQ(chat_msg, int(chat_id), key, iv)
                                        if chat_packet:
                                            await SEndPacKeT(whisper_writer, online_writer, 'ChaT', chat_packet)
                                    except Exception as chat_err:
                                        print(f"Chat error: {chat_err}")
                                    
                                    # Optional: Leave after ghost join (remove if you want to stay)
                                    await asyncio.sleep(1)
                                    leave_packet = await ExiT(None, key, iv)
                                    if leave_packet:
                                        await SEndPacKeT(whisper_writer, online_writer, 'OnLine', leave_packet)
                                    ...
                                    await safe_send_message(response.Data.chat_type, 
                                        f"[B][C][00FF00]✅ Ghost [FFFFFF]{ghost_name}[00FF00] joined [FFFFFF]{team_code}[00FF00]!", 
                                        uid, chat_id, key, iv)
                                else:
                                    await safe_send_message(response.Data.chat_type, 
                                        f"[B][C][FF0000]❌ Failed to create ghost packet!", 
                                        uid, chat_id, key, iv)
...
"""
                                ...
                                try:
                                    url = f"https://rizerxghostbd.onrender.com/x/{teamcode}/MG24_GAMER"
...
# NEW ATTACK COMMAND (AUTO STOP AFTER 1 SECOND)
                        ...
                        if inPuTMsG.startswith('/exit'):
                            ...
                            leave_room = await Mg24LeaveRoom(uid,key,iv)
                            await SEndPacKeT(whisper_writer , online_writer , 'OnLine' , leave_room)

                            # SUCCESS MESSAGE
                            success_message = f"[B][C][00FF00]✅ SUCCESS! Left the squad & room successfully!\n"
                            await safe_send_message(response.Data.chat_type, success_message, uid, chat_id, key, iv)
...
# ==================== /coin COMMAND ====================
                        if inPuTMsG.strip().startswith('/coin'):
                            ...
                            coin_message = (
                                ...
                                f"║ 🙋 Flipped by: [00FF00]MG24 GAMER\n"
                                f"║ ▪ Try again: /coin\n"
                                f"╚════════════════════╝"
                            )
...
# In TcPChaT function, update /status command:
                        ...
                        # FIXED MG24 TCP HELP MENU SYSTEM - Now detects commands properly
                        # IMPROVED MG24 TCP HELP MENU SYSTEM - AUTOMATIC MULTI-PART
                        # IMPROVED MG24 TCP HELP MENU SYSTEM - TREE STYLE FORMAT
                        if inPuTMsG.strip().lower() in ("help","/help","hi","hello","commands"):
                            print(f"Help command detected from UID: {uid}")

                            p1 = """[C][B][FF00FF]━━━━━━━━━━━━━━
[FFFFFF]        DIBOXE TCP HELP MENU 1
[FF00FF]━━━━━━━━━━━━━━
...
[FF00FF]━━━━━━━━━━━━━━"""
                            ...
                            p10 = """[C][B][FF00FF]━━━━━━━━━━━━━━
[FFFFFF]        DIBOXE TCP HELP MENU 10
[FF00FF]━━━━━━━━━━━━━━
...
[FF00FF]━━━━━━━━━━━━━━"""
                            ...
                            p11 = """[C][B][FF00FF]━━━━━━━━━━━━━━
[FFFFFF]        DIBOXE TCP HELP MENU 11
[FF00FF]━━━━━━━━━━━━━━
...
[FF00FF]━━━━━━━━━━━━━━"""
                            ...
                            p12 = """[C][B][FF00FF]━━━━━━━━━━━━━━
[FFFFFF]        DIBOXE TCP HELP MENU 12
[FF00FF]━━━━━━━━━━━━━━
...
[FF00FF]━━━━━━━━━━━━━━"""
                            ...
                            p14 = """[C][B][FF00FF]━━━━━━━━━━━━━━
[FFFFFF]        DIBOXE TCP HELP MENU 14
[FF00FF]━━━━━━━━━━━━━━
...
[FF00FF]━━━━━━━━━━━━━━"""
                            ...
                            p15 = """[C][B][FF00FF]━━━━━━━━━━━━━━
[FFFFFF]        DIBOXE TCP HELP MENU 15
[FF00FF]━━━━━━━━━━━━━━
